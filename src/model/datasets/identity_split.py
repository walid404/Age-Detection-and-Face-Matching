...
def identity_aware_split(
    ...
    """
    Identity-aware split on DataFrame level which returns indices.
...
def dataset_identity_aware_split(
    dataset: Dataset,
    train_ratio: float,
    val_ratio: float,
    seed: int = 42
    ...
    train_idx, val_idx, test_idx = identity_aware_split(df, train_ratio, val_ratio, seed)
...
def dataframe_identity_aware_split(
    df: pd.DataFrame,
    train_ratio: float,
    val_ratio: float,
    seed: int = 42,
